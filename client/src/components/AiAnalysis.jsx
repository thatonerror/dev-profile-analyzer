const AIAnalysis = ({ analysis }) => {
  return (
    <div className="bg-gradient-to-br from-emerald-500/20 via-teal-500/20 to-blue-500/20 border border-emerald-400/40 rounded-2xl p-8 shadow-2xl">
      <div className="text-center mb-8">
        <div className="w-20 h-20 bg-gradient-to-r from-emerald-400 to-teal-400 rounded-2xl mx-auto flex items-center justify-center mb-4 shadow-lg">
      ...
      {/* Summary */}
      <div className="mb-8 p-8 bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20">
        ...
        <p className="text-lg leading-relaxed text-gray-100">{analysis.summary || 'Analysis summary will appear here...'}</p>
      </div>
            ...
            {analysis.technicalStrengths?.length > 0 ? (
              analysis.technicalStrengths.map((strength, idx) => (
                <div key={idx} className="flex items-start p-4 bg-gradient-to-r from-blue-500/10 to-purple-500/10 rounded-xl border border-blue-500/30 hover:scale-105 transition-all">
                  ...
                  <span className="font-medium text-gray-100">{strength}</span>
                </div>
              ))
            ) : (
              <p className="text-gray-400 italic">Strengths will appear after analysis</p>
            )}
          </div>
        </div>
        ...
        <div className="p-8 bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20">
          ...
          <div className="space-y-3">
            {analysis.improvementTips?.length > 0 ? (
              ...
              <p className="text-gray-400 italic">Tips will appear after analysis</p>
            )}
          </div>
        </div>
      </div>

      <div className="text-center p-6 bg-gradient-to-r from-emerald-500/30 via-teal-500/30 to-blue-500/30 rounded-2xl border border-emerald-400/50 shadow-xl">
        <p className="text-xl font-bold text-emerald-200 mb-2">
          🎯 Ready for Interviews!
        </p>
        <p className="text-emerald-100">Follow the tips above to level up your profile 🚀</p>
      </div>
    </div>
  );
};
